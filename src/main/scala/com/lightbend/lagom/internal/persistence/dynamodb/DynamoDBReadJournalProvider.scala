...
private[lagom] class ScalaDSLDynamoDBReadJournal(system: ExtendedActorSystem)
  extends EventsByTagQuery
    with ReadJournal {
  override def eventsByTag(tag: String, offset: Offset): Source[EventEnvelope, NotUsed] =
    DynamoDBAkkaPersistenceEventsProviderActor.eventsByTag(tag, offset)(system)
}
private[lagom] class JavaDSLDynamoDBReadJournal(system: ExtendedActorSystem)
  extends JEventsByTagQuery
    with JReadJournal {
